...
"""
function tlm(fx::Function, fp::Function, dx0::Vector, traj::AbstractArray,
             tvec::AbstractArray, p::Vector, pidx::Int=0)
    ...
    if pidx == 0
        tlm_rhs!(df, dx, x, t) = fx(df, dx, x, p, t)
    end

    println(size(tlm_traj, 1))
    println(size(tlm_traj, 2))
    nsteps = size(tvec, 1)
    for i=1:(nsteps - 1)
        t = tvec[i]
        x = @view(traj[:, i])
